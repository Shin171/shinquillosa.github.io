...
const TechStacks = () => {
  ...
  const technologies = [
    ...
    { name: 'PostgreSQL', Icon: Database, category: 'Database', proficiency: 82 },
    ...
    <section id="tech-stacks" className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto animate-slide-up-fade" style={{ animationDelay: '0.2s' }}>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-16">
          ...
          <div className="relative">
            <div className="w-[400px] h-[400px] relative animate-slide-up-fade" style={{ animationDelay: '0.4s' }}>
              {/* Center Circle */}
              ...
              <div className="absolute inset-0 animate-rotate-globe" style={{ transformStyle: 'preserve-3d' }}>
                    ...
                    >
                      <div 
                        className={`w-16 h-16 bg-gradient-card border-2 border-brown-light/50 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-125 hover:border-brown-accent hover:bg-brown-dark/30 animate-slide-up-fade ${
                          hoveredTech === tech.name ? 'animate-bounce-in scale-125 border-brown-accent' : ''
                        }`}
                      ...
                      {hoveredTech === tech.name && (
                        <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-card border border-brown-light/30 rounded-lg p-3 whitespace-nowrap animate-fade-in shadow-lg">
                          ...
                          <div className="text-xs text-brown-accent">{tech.category}</div>
                          <div className="text-xs text-muted-foreground mt-1">{tech.proficiency}% proficiency</div>
                        </div>
                      )}
                    </div>
                  );
              ...
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[320px] h-[320px] border border-brown-light/10 rounded-full animate-spin-slow" style={{ animationDirection: 'reverse' }}></div>
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[280px] h-[280px] border border-brown-accent/20 rounded-full animate-spin-slow" style={{ animationDelay: '1s' }}></div>
            </div>
          </div>
                ...
                >
                  <div className="flex justify-between items-center">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-xs px-2 py-1 bg-brown-dark/50 rounded-full text-brown-accent border border-brown-light/30">
                        {tech.category}
                      </span>
                    </div>
                    <span className="text-sm text-brown-accent font-medium">{tech.proficiency}%</span>
                  </div>
